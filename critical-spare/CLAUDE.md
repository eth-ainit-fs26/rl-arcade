# critical-spare : the Critical Spare MDP viz (a front-screen rl-arcade cartridge)

A browser-only, no-build, file://-safe educational visualization of the
**Critical Spare** machine-maintenance MDP for a **manager audience**. It is a
**front-screen cartridge of the AI in Industry rl-arcade** (the
`eth-ainit-fs26/rl-arcade` repo: an 80s arcade start screen of games plus a
THEORY GAMES bundle). Sibling cartridges in that arcade include `gamblers-ruin/`,
`press-your-luck/`, and `pokemon-battle/`; this directory carries the same
light-pixel house theme and engine conventions. The viz walks the canonical
**13-scene arc** (title -> tutorial -> playtest -> formalization -> policy ->
trajectory -> return -> Q* -> Bellman -> DP -> DP-caveat -> SARSA -> recap).

**Design source of truth (NOT in this repo).** The full spec (exact numbers,
scene-by-scene plan) and the 13-scene template live in a **design-spec folder
that was NOT copied into this repo**: `mdp-gallery/` is part of the **SML
`classic_rl` repo**, where this cartridge was originally built. The relevant
files there are `mdp-gallery/brainstorm/proposals/08-critical-spare.md` (the
spec) and `mdp-gallery/reference/pokemon-arc.md` (the 13-scene DNA). Any
`../mdp-gallery/...` path mentioned below or in old notes refers to that external
SML repo, not to anything present here. This cartridge was built by mirroring the
multi-file structure, click-step scene engine, KaTeX vendoring, chiptune music,
EN+JP i18n, and DP-verified precompute of `gamblers-ruin/` (the most recent
fully-conformant sibling, whose `style.css` IS the arcade's light Press Start 2P
pixel house theme).
...
critical-spare/
  ...
  data/datasets.js        GENERATED by precompute (window.DATA, ~34 KB)
  precompute/build-datasets.js   the rigor gate (see below)
  vendor/katex/           KaTeX vendored (no CDN): katex.min.js/.css + fonts/
  assets/fonts/           PressStart2P-Regular.woff2 (bundled, offline)
  .gitignore              .shots_tmp/, *.pid, .DS_Store
```
...
optional network asset, pulled from Google Fonts in `index.html`; it falls back
...
- `window.Machine` is aliased to `window.Battle`, and `window.Levers` to
  ...
  New scene code should read `window.Machine` / `window.Levers`.
...
2. **Optimal policy == the spec twist grid EXACTLY** (HEALTHY: RUN RUN RUN;
...
Payload ~34 KB (V*, Q*, policy, per-sweep DP fill frames, 10 SARSA snapshots, a
greedy-return curve, a demo quarter, return-distribution data, recap copy, KaTeX
strings). Recap badge keys: **MDP / POL / RTN / Q* / DP / TD**.

## SARSA, not the SARSA-vs-Q-learning split (the model-free choice)

The build requirement asked: if on-policy SARSA converges to the optimal policy,
use SARSA; if not (as happened for Gambler's Ruin), show the honest
SARSA-vs-Q-learning split. **Here SARSA converges**, so this cartridge takes the
**SARSA path** (scene 11 = "SARSA", badge "TD", like most siblings). This was
established empirically before building: on THIS MDP the optimal lever is
**decisive** in every cell (e.g. ORDER beats RUN by ~0.75 at (AGING,0); REPLACE
clearly beats RUN with a spare in hand), so the on-policy value of the eps-soft
policy still ranks the optimal lever first. With a GLIE-annealed epsilon
(0.5 -> 0.05) and a Robbins-Monro step size (alpha = 1/(1+visits)^0.75),
on-policy SARSA recovers the **exact** DP twist grid on all 9 states across every
seed tried (10/10), and its greedy return from (HEALTHY,0) converges to V*
(9.15 vs 9.137). The precompute ASSERTS SARSA == DP on all 9 states. (Gambler's
Ruin needed the split only because its Q-gaps were sub-0.02 and the on-policy
bias was systematic; that condition does not hold here.) `sarsa.js` still ships
an off-policy `qLearningUpdate` for parity / a future contrast, but it is not
used.
...
`sarsa.js`. ~47 s (400k SARSA episodes, seed 20260615; the convergence story
wants the long run). Deterministic.
...
## Parity checklist (vs the rl-arcade siblings gamblers-ruin / press-your-luck)
...
- [x] Full EN + JP (Japanese, all 13 scenes; DotGothic16 bitmap font for kana).
...
## Registering this cartridge in the arcade

When this is wired into the rl-arcade start screen, the game (list entry, name,
blurb, sprite, Japanese strings) is registered in the **root `cartridges.js`** of
`eth-ainit-fs26/rl-arcade`; the start-screen engine is the root `js/arcade.js`,
and the project overview lives in the root `CLAUDE.md`. This directory is a
self-contained cartridge; nothing here needs to import from the arcade root.
