...
(function () {
  ...
  const STRINGS = {
    en: {
      ...
      'traj.btn.step':        '▶ STEP THE PATH',
      'traj.btn.play':        '▶▶ PLAY',
      'traj.btn.sample':      '▶ SAMPLE A PATH',
      'traj.btn.reset':       'RESET',
      'traj.status.step':     'step',
      'traj.status.over':     'EPISODE OVER',
      'traj.status.hint':     'One trajectory = one root-to-leaf PATH. SAMPLE lights a whole path; STEP walks it one step at a time.',
      'traj.tree.foot':       'one trajectory τ = one root-to-leaf PATH through this tree.',
      'traj.tree.caption':    'The trajectory TREE: every possible path from <b>{state}</b> under <b>{move}</b>. Nodes are states; each edge carries the action, its probability p, and the reward r; each leaf carries G<sub>t</sub> = sum of rewards on the path.',
      'traj.derived.label':   'THE LIT PATH, AS THE OLD TAPE τ = (S, A, R, …)',
      'traj.derived.empty':   '(press SAMPLE or STEP to light a path; it appears here as the flat tape)',
      'traj.derived.g':       'G<sub>t</sub> of this path = <b>{g}</b>',
      ...
      'obj.var.target':       'target E[G] = <b>{target}</b>',
      'obj.var.target_tag':   'E[G] = {target}',
      'obj.eg.label':         'E[G<sub>t</sub>] = the WEIGHTED SUM over the leaves of this trajectory tree (root <b>{state}</b>, action <b>{move}</b>). Hover a ledger row to light its path.',
      'obj.eg.tie':           'The weighted leaf sum is <b>E[G<sub>t</sub>] = {eg}</b>. Because we acted optimally after the first move, that IS <b>Q*({state}, {move}) = {eg}</b>. The return G<sub>t</sub>, the objective E[G<sub>t</sub>], and the value Q* are the same picture.',
      ...
      'qstar.state_s':        'state s',
      'qstar.col.action':     'action a',
      'qstar.col.qstar':      'Q*(s, a)',
      ...
      'dp.backup.title':      'THE BACKUP IS THE DEPTH-1 TRAJECTORY TREE',
      'dp.backup.hint':       '(click to expand)',
      'dp.backup.lead':       'Take action <b>THUNDER</b> in <b>FULL / MID</b> for ONE step, then bootstrap: a frontier leaf carries G<sub>t</sub> = r + V(s′) (collected reward + value-to-go). The weighted leaf sum of this one-ply tree is exactly the Bellman backup that fills the cell below.',
      'dp.backup.tie':        'Weighted leaf sum = <b>{eg}</b> = <b>Q*({state}, {move})</b>. This is the same E[G<sub>t</sub>] = sum of P·G<sub>t</sub> ledger from two scenes ago, now ONE ply deep with bootstrapped leaves: that is the Bellman backup.',
      'dp.btn.step':          '▶ STEP',
      'dp.btn.run':           'RUN ALL',
      'dp.btn.reset':         'RESET',
    ...
    jp: {
      ...
      'traj.btn.step':        '▶ パスを すすめる',
      'traj.btn.play':        '▶▶ さいせい',
      'traj.btn.sample':      '▶ パスを サンプル',
      'traj.btn.reset':       'リセット',
      'traj.status.step':     'ステップ',
      'traj.status.over':     'エピソード しゅうりょう',
      'traj.status.hint':     'ひとつの きせき = ねもとから はっぱまでの ひとつの パス。 SAMPLEで パス ぜんたいを てんとう、 STEPで 1ステップずつ すすむ。',
      'traj.tree.foot':       'ひとつの きせき τ = この きの ねもとから はっぱまでの ひとつの パス。',
      'traj.tree.caption':    'きせきの き： <b>{state}</b> で <b>{move}</b> を とった ときの すべての パス。 ノードは じょうたい。 えだは こうどう・かくりつ p・ほうしゅう r を はこぶ。 はっぱは G<sub>t</sub> = パス じょうの ほうしゅうの ごうけい を もつ。',
      'traj.derived.label':   'てんとう した パス を むかしの テープ τ = (S, A, R, …) として',
      'traj.derived.empty':   '（SAMPLE か STEP で パスを てんとう。 ここに たいらな テープ として でる）',
      'traj.derived.g':       'この パスの G<sub>t</sub> = <b>{g}</b>',
      ...
      'obj.var.target':       'もくひょう E[G] = <b>{target}</b>',
      'obj.var.target_tag':   'E[G] = {target}',
      'obj.eg.label':         'E[G<sub>t</sub>] = この きせきの きの はっぱ ぜんたいの じゅうみつわ（ねもと <b>{state}</b>、 こうどう <b>{move}</b>）。 だいちょうの ぎょうに カーソルを あてると その パスが てんとうする。',
      'obj.eg.tie':           'じゅうみつわ は <b>E[G<sub>t</sub>] = {eg}</b>。 さいしょの いっての あと さいてきに あそんだ ので、 これは <b>Q*({state}, {move}) = {eg}</b>。 リターン G<sub>t</sub>、 もくてき E[G<sub>t</sub>]、 かち Q* は おなじ え。',
      ...
      'qstar.state_s':        'じょうたい s',
      'qstar.col.action':     'こうどう a',
      'qstar.col.qstar':      'Q*(s, a)',
      'qstar.argmax_tag':     'argmax · π*(s)',
      ...
      'dp.backup.title':      'こうしんは しんさ 1 の きせきの き',
      'dp.backup.hint':       '（クリックで ひらく）',
      'dp.backup.lead':       '<b>まんたん / なかほど</b> で <b>かみなり</b> を 1ステップ とり、 そのあと ブートストラップ： フロンティアの はっぱは G<sub>t</sub> = r + V(s′)（あつめた ほうしゅう ＋ さきの かち）を もつ。 この 1プライの きの じゅうみつわ が、 したの セルを うめる ベルマン こうしん そのもの。',
      'dp.backup.tie':        'じゅうみつわ = <b>{eg}</b> = <b>Q*({state}, {move})</b>。 これは 2つまえの シーンの E[G<sub>t</sub>] = sum of P·G<sub>t</sub> だいちょう が、 1プライ ふかく、 はっぱを ブートストラップ した もの： それが ベルマン こうしん。',
      'dp.btn.step':          '▶ ステップ',
      'dp.btn.run':           'ぜんぶ じっこう',
      'dp.btn.reset':         'リセット',
